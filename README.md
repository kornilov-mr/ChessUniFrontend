...
## Description
The project is a frontend part for chess, which was striped from the backend part for an internship project.
...
* Then the CSS files are processed by the PostCSS (which is responsible for optimization, compatibility with different browsers, Autoprefixes, etc.)
...
running the build command and finding the CSS file in the build folder *build/static/css*.
